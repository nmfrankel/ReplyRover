...
import { Zmanim,Zman, formatDate, formatTime, remindGate } from './utils.js'
...
app.post('/zmanim', async (req, res) => {
	...
	// )
	const geocoding = await fetch(
		`https://maps.googleapis.com/maps/api/geocode/json?address=${location}&region=us&key=AIzaSyComHeNc7JvWgN39zimcr8sdKjgCDWDnH4`
	)
	...
	// )
	const elevationAPI = await fetch(
		`https://maps.googleapis.com/maps/api/elevation/json?locations=${coordinates.lat},${coordinates.lng}&key=AIzaSyComHeNc7JvWgN39zimcr8sdKjgCDWDnH4`
	)
